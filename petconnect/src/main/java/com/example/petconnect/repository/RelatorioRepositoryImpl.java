...
public class RelatorioRepositoryImpl implements RelatorioRepository {
    ...
    public List<RelatorioVendasDTO> relatorioVendasPorMes(LocalDate dataInicio) {
        ...
        String jpql = "SELECT YEAR(a.dataAdocao), MONTH(a.dataAdocao), COUNT(a) " +
                "FROM Adocao a WHERE a.dataAdocao >= :inicio AND a.status = com.example.petconnect.entity.enums.StatusAdocao.APROVADA " +
                "GROUP BY YEAR(a.dataAdocao), MONTH(a.dataAdocao) " +
                "ORDER BY YEAR(a.dataAdocao), MONTH(a.dataAdocao)";
